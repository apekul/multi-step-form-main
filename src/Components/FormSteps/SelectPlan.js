...
const plans = [
  { name: "Arcade", price_month: 9, price_year: 108, icon: arcade },
  { name: "Advanced", price_month: 12, price_year: 144, icon: advanced },
  { name: "Pro", price_month: 15, price_year: 180, icon: pro },
];
...
const SelectPlan = ({ planData, setPlanData, setCurrentStep }) => {
  ...
  const toggleType = () => {
    setType((prevType) => {
      ...
      const price = newType === "monthly" ? item.price_month : item.price_year;

      setPlanData((prev) => ({ ...prev, type: newType, price }));
      ...
      <div className="items-start justify-between flex flex-col flex-1">
        <ul className="w-full mt-[2rem] grid grid-flow-row grid-cols-3 gap-5">
          {plans.map((item, index) => (
            <li
              key={index}
              className={`h-[10rem] px-3 pb-3 pt-[1.10rem] border-2 rounded-md flex flex-col justify-between cursor-pointer  hover:border-[#AFAAD6]
              ${
            ...
            >
              ...
              <span>
                <p className="text-[#143257] font-bold">{item.name}</p>
